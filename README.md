...
- **Battery Low**: one binary sensor per battery-powered device, on while RainPoint reports that device's cells as low. It answers low or not rather than how much is left, because a charge level is not something RainPoint's cloud sends: the reading behind it is a condition flag. This replaces the battery percentage entities earlier versions built, which could only ever read 100% or nothing at all; a repair card offers to remove the ones left in your registry.
