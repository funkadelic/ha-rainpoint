...
- **Sensor entities**: one per measurement (moisture, temperature, rain, CO2, etc.) plus two disabled-by-default diagnostic sensors. **Raw Payload** shows the raw hex data from the API. **Catalog Coverage** shows how many readings were understood from that data, and lists alongside it every reading RainPoint's own product data says the model can send, so you can see what this integration does not yet read from a device it otherwise supports. A device that returns no readings at all gets a single **Not Reporting** diagnostic entity instead, and neither of the other two, because there is no payload to show.
