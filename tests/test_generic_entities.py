...
class TestEvaluateGenericGate:
    ...
    def test_model_present_in_catalog_but_declaring_nothing_is_not_reported_as_absent(self, monkeypatch):
        ...
        monkeypatch.setattr(generic_entities_module, "get_catalog_entry", lambda model, model_code=None: [])
        monkeypatch.setattr(generic_entities_module, "get_catalog_port_number", _port_number_spy)

        result = evaluate_generic_gate(FAKE_MODEL, "code-a")
        ...
        assert result.port_number == 5
        assert captured == [(FAKE_MODEL, "code-a")]
    ...
    def test_exactly_one_curated_status_datapoint_passes(self, monkeypatch):
        ...
        result = evaluate_generic_gate(FAKE_MODEL, "code-b")
        ...
        assert captured == [(FAKE_MODEL, "code-b")]
    ...
    def test_hand_written_model_states_hand_written_reason(self):
        ...
        assert result.passed is False
        assert result.datapoints == []
        assert result.blocked_by == ("this model already has a hand-written decoder, so it never uses generic entities",)
...
class TestBadPortReasonExactWording:
    ...
    def test_two_offenders_produce_the_exact_sorted_sentence(self):
        ...
        assert reason == (
            "these readings don't declare a usable port number, so they can't be turned into entities: STA_RH, STA_TEM"
        )
...
class TestDuplicatePortReasonBehavior:
    ...
    def test_an_earlier_rejected_entry_does_not_stop_a_later_duplicate_from_being_found(self):
        ...
        dp_entries = [
            _dp("STA_TEM", dp_port="bad"),
            _dp("STA_RH", dp_port=0, dp_code=1),
            _dp("STA_RH", dp_port=0, dp_code=2),
        ]
        reason = generic_entities_module._duplicate_port_reason(dp_entries)
    ...
    def test_exact_message_is_sorted_by_identity_then_port_as_strings(self):
        ...
        dp_entries = [
            ...
            _dp("A", dp_port=2, dp_code=3),
            _dp("A", dp_port=2, dp_code=4),
            _dp("B", dp_port=2, dp_code=5),
            _dp("B", dp_port=2, dp_code=6),
        ]
        reason = generic_entities_module._duplicate_port_reason(dp_entries)
        # String-sorted, so "10" sorts before "2" (compares "1" < "2" on the
        # first character), which is what pins the key function's shape.
        assert reason == (
            "the catalog declares the same reading more than once for the same port, so its value would be ambiguous: "
            "A on port 10, A on port 2, B on port 2"
        )
...
class TestDpCodeSortKeyNonIntTieBreak:
    ...
    def test_two_distinct_non_int_codes_sort_by_their_own_string_not_a_shared_placeholder(self):
        ...
        str(None) == "None" sorts before "aaa" and "zzz" (uppercase 'N' precedes
        lowercase letters in ASCII), so a correct sort reorders the input
        ["zzz", "aaa"] to ["aaa", "zzz"] around it. A tiebreak collapsed onto a
        shared placeholder would instead leave the input order untouched.
        """
        keys = sorted([None, "zzz", "aaa"], key=generic_entities_module._dp_code_sort_key)
        assert keys == [None, "aaa", "zzz"]
...
class TestDpCodeReasonsExactWordingAndScanContinuation:
    ...
    def test_an_earlier_non_dict_entry_does_not_stop_a_later_duplicate_from_being_found(self):
        ...
        reasons = generic_entities_module._dp_code_reasons(raw_entry)
        assert any("dpCode" in reason and "9" in reason for reason in reasons)

    def test_an_earlier_unusable_dp_code_does_not_stop_a_later_duplicate_from_being_found(self):
        """The unhashable-dpCode guard also uses `continue`, not `break`, after recording it."""
        raw_entry = [
            {"identity": "STA_ALARM", "dpCode": [1, 2], "dpPort": 0},
            _dp("STA_TEM", dp_port=0, dp_code=9),
            _dp("STA_RSSI", dp_port=1, dp_code=9),
        ]
        reasons = generic_entities_module._dp_code_reasons(raw_entry)
        assert any("usable datapoint code" in reason for reason in reasons)
        assert any("dpCode" in reason and "9" in reason for reason in reasons)
    ...
    def test_unusable_message_exact_wording(self):
        ...
        reasons = generic_entities_module._dp_code_reasons(raw_entry)
        assert reasons == [
            "these readings don't declare a usable datapoint code, so they can't be matched to a decoded value: STA_AAA, STA_ZZZ"
        ]
    ...
    def test_duplicate_message_exact_wording(self):
        ...
        reasons = generic_entities_module._dp_code_reasons(raw_entry)
        assert reasons == [
            "the catalog reuses one datapoint code for more than one reading, so those readings can't be told apart: "
            "dpCode 9 (STA_RSSI, STA_TEM)"
        ]
...
class TestUnresolvedVariantReasonExactWording:
    ...
    def test_reported_code_absent_from_the_catalog_joins_known_codes_with_a_plain_comma(self, monkeypatch):
        ...
        assert reason == (
            "the product catalog has no entry for this device's hardware variant of REAL_MODEL "
            "(it reports 999; the catalog lists 111, 222)"
        )
    ...
    def test_no_reported_code_joins_known_codes_with_a_plain_comma(self, monkeypatch):
        ...
        reason = generic_entities_module._unresolved_variant_reason("REAL_MODEL", None)
        assert reason == (
            "the catalog lists more than one hardware variant for REAL_MODEL "
            "(111, 222) and this device did not report which one it is"
        )
...
class TestEvaluateGenericGateSortKeyUsesTheRealFields:
    ...
    def test_wrong_port_key_would_sort_by_identity_alone_this_must_not_happen(self, monkeypatch):
        ...
        monkeypatch.setattr(generic_entities_module, "get_catalog_entry", lambda model, model_code=None: dp_entries)
        monkeypatch.setattr(generic_entities_module, "get_catalog_port_number", lambda model, model_code=None: 6)

        result = evaluate_generic_gate(FAKE_MODEL, None)

        identities = [entry.get("identity") for entry in result.datapoints]
        assert identities == ["STA_TEM", "STA_RSSI"]

    def test_same_port_ties_break_alphabetically_by_identity_not_input_order(self, monkeypatch):
        """Input order is deliberately reversed from the alphabetical tiebreak."""
        dp_entries = [_dp("STA_TEM", dp_port=0, dp_code=9), _dp("STA_RSSI", dp_port=0, dp_code=10)]
        monkeypatch.setattr(generic_entities_module, "get_catalog_entry", lambda model, model_code=None: dp_entries)
        monkeypatch.setattr(generic_entities_module, "get_catalog_port_number", lambda model, model_code=None: 1)
        ...
        identities = [entry.get("identity") for entry in result.datapoints]
        assert identities == ["STA_RSSI", "STA_TEM"]
...
class TestDescribeGenericGateForwardsArguments:
    ...
    def test_forwards_model_and_model_code_unchanged(self, monkeypatch):
        ...
        monkeypatch.setattr(generic_entities_module, "evaluate_generic_gate", spy)

        describe_generic_gate("MODEL_A", "CODE_A")
...
class TestCountGenericEligibleDevicesArgumentForwardingAndIncrement:
    ...
    def test_evaluate_generic_gate_receives_this_devices_own_model_and_model_code(self, monkeypatch):
        ...
        count_generic_eligible_devices(data)
    ...
    def test_eligible_count_increments_rather_than_resets_per_device(self, monkeypatch):
        ...
        monkeypatch.setattr(generic_entities_module, "evaluate_generic_gate", lambda model, model_code=None: passing_result)
        data = {
            "sensors": {
        ...
        assert count_generic_eligible_devices(data) == (2, 2)
...
class TestBuildGenericEntitiesForwardsConstructorArguments:
    ...
    def test_entities_carry_the_real_sensor_info_and_port_number(self, monkeypatch):
        ...
        entities = build_generic_entities(coordinator, "100_200_1", sensor_info, "slug123")

        assert len(entities) == 1
        entity = entities[0]
        assert entity._sensor_info is sensor_info
        # port_number(2) > 1 must have reached the entity for the zone prefix to appear.
        assert entity._attr_name == "Zone 1 Temperature (unverified)"
...
class TestRainPointGenericSensorConstructorWiring:
    ...
    def test_sensor_info_flows_through_to_the_base_class(self):
        ...
        sensor = RainPointGenericSensor(coordinator, "100_200_1", sensor_info, "100_200_1", dp_entry, 1)

        assert sensor._sensor_info is sensor_info

    def test_zone_appears_at_port_number_two_the_lowest_multi_port_value(self):
        """Pins `> 1`, not `> 2`, and `>= 1`, not `> 1` or `>= 2`, on the dp_port side."""
        dp_entry = _dp("STA_TEM", dp_port=1, dp_code=9)
        sensor = _make_generic_sensor(dp_entry, port_number=2)
        assert sensor._attr_name == "Zone 1 Temperature (unverified)"

    def test_no_zone_at_port_number_one_even_with_a_dp_port_of_one(self):
        """Pins `port_number > 1`, not `>= 1`: a single-port variant never gets a zone prefix."""
        dp_entry = _dp("STA_TEM", dp_port=1, dp_code=9)
        sensor = _make_generic_sensor(dp_entry, port_number=1)
        assert sensor._attr_name == "Temperature (unverified)"

    def test_unit_state_class_and_precision_are_read_from_the_spec_not_hardcoded(self, monkeypatch):
        """The constructor's unconditional assignment must copy the spec's value, not bake in a literal."""
        ...
        fake_spec = GenericSensorSpec(
            ...
            transform=lambda raw: float(raw),
            valid_range=(0.0, 100.0),
            precision=3,
            widths=frozenset({1}),
        )
        ...
        assert sensor._attr_native_unit_of_measurement == "fake_unit"
